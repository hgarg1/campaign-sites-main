...
const PORTAL_PATH = '/tenant-chooser'
const LOGIN_PATH = '/login'
const APP_NAME = 'CampaignSites Tenant Admin'
...
const store = new Store<AppStore>({
  defaults: { token: null, selectedOrgId: null, windowBounds: null, windowMaximized: false },
  encryptionKey: 'cs-tenant-admin-2024'
})
...
function attachPortalView(orgId?: string | null): void {
  ...
  const resolvedOrgId = orgId ?? store.get('selectedOrgId')
  const portalUrl = resolvedOrgId
    ? `${PRODUCTION_URL}/tenant/${resolvedOrgId}`
    : `${PRODUCTION_URL}${PORTAL_PATH}`

  portalView.webContents.loadURL(portalUrl)
}
...
ipcMain.handle('select-org', (_event, orgId: string) => {
  store.set('selectedOrgId', orgId)
  attachPortalView(orgId)
  mainWindow?.webContents.send('org-selected', { orgId })
  return { success: true }
})
...
app.whenReady().then(() => {
  electronApp.setAppUserModelId('com.campaignsites.tenant-admin')

  app.setAsDefaultProtocolClient('campaignsites-tenant')
