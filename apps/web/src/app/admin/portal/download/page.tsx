...
function getSystemAdminAssets(assets: ReleaseAsset[]) {
  return assets.filter(
    (a) =>
      a.name.toLowerCase().includes('system') ||
      a.name.toLowerCase().includes('campaignsites-system-admin')
  );
}
...
export default function DownloadPage() {
  ...
  const systemAdminAssets = release ? getSystemAdminAssets(release.assets) : [];

  return (
    <AdminLayout title="Desktop App" subtitle="Download the native desktop application">
      <div className="max-w-3xl mx-auto p-6 space-y-8">
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold text-white">Desktop App</h1>
          <p className="text-slate-400 mt-1">
            Download the native CampaignSites System Admin desktop application for your operating
            system.
          </p>
        </div>
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
          ...
          <div className="flex items-center gap-4 p-6 border-b border-slate-700">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="text-lg font-bold text-white">CampaignSites System Admin</h2>
              <p className="text-slate-400 text-sm">
                Native desktop app with OS integration, auto-updates, and offline capability.
              </p>
              {release && (
          ...
          <div className="p-6 border-b border-slate-700">
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { icon: '🌐', text: 'Protocol handler (campaignsites-admin://)' },
              ].map(({ icon, text }) => (
          ...
          <div className="p-6">
            ...
            {!loading && !error && systemAdminAssets.length === 0 && (
            ...
            {!loading && systemAdminAssets.length > 0 && (
              <div className="space-y-3">
                <p className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-4">
                  Download for your platform
                </p>
                {systemAdminAssets.map((asset) => {
