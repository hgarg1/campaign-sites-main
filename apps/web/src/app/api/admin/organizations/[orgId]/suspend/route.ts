/**
 * API endpoint to suspend an organization
...
export async function POST(
  ...
  try {
    ...
    const canSuspend = await hasSystemAdminPermission(
      userId,
      'system_admin_portal:organizations:suspend'
    );
    if (!canSuspend) {
      await logSystemAdminAction({
        action: 'ORGANIZATION_SUSPEND_DENIED',
