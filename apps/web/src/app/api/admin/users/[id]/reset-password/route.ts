...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Check permission
    const hasPermission = await hasSystemAdminPermission(adminId, 'system_admin_portal:users:password_reset');
    if (!hasPermission) {
      ...
      return NextResponse.json(
        { error: 'Insufficient permissions for system_admin_portal:users:password_reset' },
        { status: 403 }
      );
    }
    ...
    // Generate reset token (in production, this should be saved with expiration)
    // For now, return a message that email would be sent
    // TODO: Integrate with email service to send actual reset email
    ...
    return NextResponse.json(
      {
        ...
        data: { id: targetUser.id, email: targetUser.email },
