...
export async function PATCH(
  ...
  const denied = await enforceSystemPolicy(params.orgId, 'integrations', 'update');
  ...
  try {
    ...
    const updated = await (prisma as any).organizationIntegration.update({
...
export async function DELETE(
  ...
  const denied = await enforceSystemPolicy(params.orgId, 'integrations', 'delete');
  if (denied) return denied;

  try {
    await (prisma as any).organizationIntegration.delete({ where: { id: params.integrationId } });
