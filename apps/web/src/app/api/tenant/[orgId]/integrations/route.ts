...
export async function GET(req: NextRequest, { params }: { params: { orgId: string } }) {
  ...
  try {
    const integrations = await (prisma as any).organizationIntegration.findMany({
...
export async function POST(req: NextRequest, { params }: { params: { orgId: string } }) {
  ...
  const denied = await enforceSystemPolicy(params.orgId, 'integrations', 'create');
  ...
  try {
    ...
    const integration = await (prisma as any).organizationIntegration.create({
      data: { type, provider, config, organizationId: params.orgId, isActive: true },
    });
